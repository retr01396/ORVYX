...
class APIProvider(BaseCoPilotProvider):
    ...
    def __init__(self):
        ...
        self.model = os.getenv("AI_API_MODEL", "gpt-4o-mini")
        self.mode = "api"
        self.provider_name = f"AI API ({self.model})"
        self._fallback = DeterministicCoPilotProvider()
...
# ---------------------------------------------------------------------------
# Provider registry — driven by COPILOT_PROVIDER environment variable
# ---------------------------------------------------------------------------

class CoPilotManager:
    """
    Factory and runtime manager for Co-Pilot providers.
    ...
    def __init__(self):
        ...
        if mode == "claude":
            ...
            self._provider = DeterministicCoPilotProvider()
        logger.info(f"CoPilot provider active: {self._provider.provider_name}")
    ...
    def answer_question(self, request: AskRequest, context: CoPilotResponse) -> AskResponse:
        return self._provider.answer_question(request=request, context=context)
